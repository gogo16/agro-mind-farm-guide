...
const AIChatSystem = () => {
  const { fields, transactions, inventory } = useAppContext();
  ...
  const agriculturalKeywords = [
    'fermă', 'cultură', 'teren', 'recoltă', 'semințe', 'îngrășământ', 'irigare',
    'utilaje', 'tractor', 'plugul', 'meteorologie', 'sol', 'prețuri', 'vânzare',
    'profit', 'costuri', 'buget', 'inventar', 'depozit', 'stoc', 'sarcini',
    'planificare', 'sezon', 'primăvară', 'vară', 'toamnă', 'iarnă'
  ];

  const isAgricultureRelated = (message: string): boolean => {
    const lowerMessage = message.toLowerCase();
    return agriculturalKeywords.some(keyword => lowerMessage.includes(keyword)) ||
           lowerMessage.includes('agro') ||
           lowerMessage.includes('agricul');
  };
  ...
  return (
    <Card className="bg-white border-green-200 h-96">
