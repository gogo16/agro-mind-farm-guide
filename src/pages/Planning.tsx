...
const Planning = () => {
  const { fields, tasks, addTask, notifications, markNotificationAsRead } = useAppContext();
    ...
    const updateDailyNotifications = () => {
      ...
      todayTasks.forEach(task => {
        const notificationExists = notifications.some(
          n => n.type === 'task' && n.message.includes(task.title) && n.date === today
        );
                            ...
                            <div className="flex items-center space-x-2">
                              <Badge className={getStatusColor(task.status)}>
                                {task.status === 'pending' ? 'În așteptare' : 
                                 task.status === 'in-progress' ? 'În progres' : 'Completat'}
                              </Badge>
                            </div>
                            {task.description && (
