            ...
            <Card className="bg-white/80 backdrop-blur-sm border-green-200">
              ...
              <CardContent className="space-y-3">
                {loading ? <div className="text-center py-4 text-gray-500">
                  ...
                  </div> : fields.length === 0 ? <div className="text-center py-4 text-gray-500">
                  ...
                  </div> : fields.map(field => <div key={field.id} className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 cursor-pointer border border-gray-100 transition-colors" onClick={() => {
                ...
                setShowFieldInfo(true);
                      ...
                      <div className="w-4 h-4 rounded-full border-2 border-white shadow flex-shrink-0" style={{
                  backgroundColor: field.culoare || '#22c55e'
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {field.data_insamantare && <p className="text-xs text-green-600">
                            Însămânțat: {new Date(field.data_insamantare).toLocaleDateString('ro-RO')}
                          </p>}
                        {field.coordonate_gps && <p className="text-xs text-blue-600">
            ...
            <Card className="bg-white border-green-200 h-[600px]">
              <CardContent className="p-0 h-full">
                <div className="relative w-full h-full rounded-lg overflow-hidden">
                  <InteractiveMap mapType={mapType} onFieldSelect={handleFieldSelect} fields={transformedFields} />
                  ...
                  {showFieldInfo && selectedField && <div className="absolute top-4 right-4 bg-white rounded-lg shadow-lg p-4 w-64 border z-20">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        {selectedField.coordinates && <p><strong>Coordonate:</strong> {Array.isArray(selectedField.coordinates) ? `${selectedField.coordinates.length} puncte` : `${selectedField.coordinates.lat}, ${selectedField.coordinates.lng}`}</p>}
                      </div>
